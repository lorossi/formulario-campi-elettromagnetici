...
\centerline{Versione aggiornata al 16/06/2020}
\end{doublespacing}
\newpage
...
\textbf{Ipotesi:} onda su piano \(xz\). Può essere scomposta in componente TE e TM (per sovrapposizione lineare).
\begin{itemize}
	\item Componente TE
	\begin{itemize}
		\item Ha componente \(y\)
		\item Impedenza \( \eta^{TE}_n = \dfrac{\eta}{cos(\theta_n)} \)
		\item Coefficiente di riflessione \( \Gamma = \dfrac{\eta_{2}^{TE} - \eta_{1}^{TE}}{\eta_{2}^{TE} + \eta_{1}^{TE}} \)
	\end{itemize}
	\item Componente TM
	\begin{itemize}
		\item Ha componenti \(xz\)
		\item Impedenza \( \eta^{TM}_n = \eta \cdot cos(\theta_n) \)
		\item Coefficiente di riflessione \( \Gamma = - \dfrac{\eta_{2}^{TE} - \eta_{1}^{TE}}{\eta_{2}^{TE} + \eta_{1}^{TE}} \)
	\end{itemize}
\end{itemize}
\begin{itemize}
...
\subsubsection{Linea a striscia}
\begin{itemize}
...
\subsection{Linee TE in guida rettangolare}
...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item Campo magnetico \(H_\theta = \dfrac{j \omega j \mu I l}{4 \pi R \eta} e^{-j \beta R} \sin\theta\) 
